...
__host__ void cu_initialise_rng_states(int n_states,
                                       curandState *state) {
    LOGF(INFO, "Calculating optimal launch configuration for the state intialisation kernel.\n");
    int block_size = 0;
    int min_grid_size = 0;
    int grid_size = 0;
    ...
    LOGF(INFO, "Launch config set as <<<%i,%i>>>\n", grid_size, block_size);
...
//     double r = ldexp(pcg32_random_r(seed), -32);
...
// __device__ double gaussian_ziggurat(pcg32_random_t *seed) {
//   return 1.;
// }

// double h_gaussian_ziggurat(pcg32_random_t *seed) {
//   int64_t U, sign, i, j;
//   double  x, y;

//   while (1) {
//     U = pcg32_random_r(seed);
...
//       y0 = ytab[i];
//       y1 = ytab[i+1];
//       y = y1 + (y0-y1)*uniform_prng(seed);
//     } else {
//       x = PARAM_R - log(1.0-uniform_prng(seed))/PARAM_R;
//       y = exp(-PARAM_R*(x-0.5*PARAM_R)) * uniform_prng(seed);
//     }
